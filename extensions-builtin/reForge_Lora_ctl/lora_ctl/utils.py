...
def patch_model(model, forget=False, orig=None):
    ...
    except ldm_patched.modules.model_management.OOM_EXCEPTION:
...
def lora_name_to_file(name):
    filenames = ldm_patched.utils.path_utils.get_filename_list("loras")
...
def load_lbw():
    return ldm_patched.contrib.external.NODE_CLASS_MAPPINGS.get("LoraLoaderBlockWeight //Inspire")


def make_loader(filename, lbw):
    if not lbw:
        l = ldm_patched.contrib.external.LoraLoader()
