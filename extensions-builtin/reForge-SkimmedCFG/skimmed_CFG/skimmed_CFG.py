...
def get_skimming_mask(x_orig, cond, uncond, cond_scale, return_denoised=False):
    # Create a dummy tensor with the same batch and sequence dimensions as cond
    dummy = torch.zeros((cond.shape[0], cond.shape[1], 1), device=cond.device)
    
    # Use the dummy tensor for operations that don't involve x_orig
    denoised = dummy - ((dummy - uncond) + cond_scale * ((dummy - cond) - (dummy - uncond)))
    matching_pred_signs = (cond - uncond).sign() == cond.sign()
    matching_diff_after = cond.sign() == (cond * cond_scale - uncond * (cond_scale - 1)).sign()
    deviation_influence = (denoised.sign() == (denoised - dummy).sign())
...
def skimmed_CFG(x_orig, cond, uncond, cond_scale, skimming_scale):
    ...
    outer_influence, denoised = get_skimming_mask(dummy, cond, uncond, cond_scale, True)
    low_cfg_denoised_outer = dummy - ((dummy - uncond) + skimming_scale * ((dummy - cond) - (dummy - uncond)))
